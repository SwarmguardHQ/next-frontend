...
function ShowcaseView({ drone, onBack }: { drone: Drone; onBack: () => void }) {
    ...
    const [telemetry, setTelemetry] = useState(generateTelemetry(drone.status === "relay" ? 80 : 50));

    useEffect(() => {
        if (!flying) return;
        const id = setInterval(() => setTelemetry(generateTelemetry(drone.status === "relay" ? 80 : 50)), 1000);
